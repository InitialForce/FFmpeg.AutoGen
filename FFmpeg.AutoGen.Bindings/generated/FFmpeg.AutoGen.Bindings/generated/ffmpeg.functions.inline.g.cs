...
{
    if ((a + (long)2147483648UL) & ~4294967295UL)
        return (int)((a >> 63) ^ 2147483647);
    else
        return (int)a;
}
    // original body hash: /U+ZBRNpZzkmnoKEu/JkR0UCE8u5fgy80CqaHVpjdgE=
...
{
    var tmp = (long)( a.num * (long)b.den - b.num * (long)a.den);
    if (tmp != 0)
...
{
    throw new NotImplementedException("Union-based type conversion not supported in generated inline functions");
}
...
{
    return av_frame_side_data_get_c((AVFrameSideData**)sd, nb_sd, type);
}
...
{
    throw new NotImplementedException("Fixed array to pointer casting not supported in generated inline functions");
}
...
{
    AVRational r = new AVRational { num = q.den, den = q.num };
    return r;
}
...
{
    AVRational r = new AVRational { num = num, den = den };
    return r;
}
...
{
    throw new NotImplementedException("GCC built-in functions not supported in generated inline functions");
}
    // original body hash: 0TPJfAPKmXTrlDh2OnQHyxoI+SynwOsCEttRkl8pAjo=
    ...
    public static long av_sat_sub64_c(long @a, long @b)
{
    ...
    // original body hash: +GjoJf/xttRf35vH1k8P+qQu2eFS1owfRUDE4UmWJB8=
