...
internal sealed class FunctionsGenerator : GeneratorBase<ExportFunctionDefinition>
{
    ...
    protected override void GenerateDefinition(ExportFunctionDefinition function)
    {
        ...
        if (IsDynamicallyLoadedGenerationOn) GenerateDynamicallyLoadedFunction(function);
    ...
    private void GenerateDynamicallyLoadedFunction(ExportFunctionDefinition function)
    {
        ...
        using (BeginBlock())
        {
            WriteLine($"_{function.Name}_cache ??= FunctionResolver.GetFunctionDelegate<{functionDelegateName}>(\"{function.LibraryName}\", \"{function.Name}\", ThrowErrorIfFunctionNotFound);");
            WriteLine($"if (_{function.Name}_cache == null)");
            WriteLine("    throw new NotSupportedException();");
            var returnCommand = function.ReturnType.Name == "void" ? string.Empty : "return ";
            WriteLine($"{returnCommand}_{function.Name}_cache({parameterNames});");
        }
        WriteLine();

        // Generate ref overloads for DynamicallyLoaded functions too
        GenerateRefOverloadsForDynamicallyLoaded(function);
    }

    private void GenerateRefOverloadsForDynamicallyLoaded(ExportFunctionDefinition function)
    {
        // Find parameters that are double pointers and could benefit from ref overloads
        var doublePointerParams = function.Parameters
            .Where(p => p.Type.Name.EndsWith("**") && !p.ByReference && !p.IsConstant)
        ...
        for (int i = 1; i < (1 << paramCount); i++) // Start from 1 to skip the original combination
        {
            ...
            // Generate ref overload that calls the original DynamicallyLoaded function
            WriteLine($"public static {function.ReturnType.Name} {function.Name}({refParameters})");
            WriteLine("{");
